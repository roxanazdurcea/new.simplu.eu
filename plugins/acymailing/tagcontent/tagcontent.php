...
class plgAcymailingTagcontent extends JPlugin
{
	...
	?>
		...
		<!--
			...
			function updateTag()
			{
				...
				if(document.adminForm.contentformat && document.adminForm.contentformat.value){ otherinfo += '| format:'+document.adminForm.contentformat.value; }
	...
	?>
		...
		<?php } ?>
			...
			<?php } ?>
				...
				for(var i=0; i < document.adminForm.contentformatauto.length; i++){
					if(document.adminForm.contentformatauto[i].checked){ tag += '| format:'+document.adminForm.contentformatauto[i].value; }
				}
				...
				if(document.adminForm.contentformatauto && document.adminForm.contentformatauto.value){ tag += '| format:'+document.adminForm.contentformatauto.value; }
	...
	private function _replaceContent(&$tag){
		...
		if(!empty($tag->cattitle) && ((!strpos($article->catid, ',') && $this->currentcatid != $article->catid) || (strpos($article->catid, ',') && !in_array($this->currentcatid, explode(',', $article->catid))))){
			...
			if(!empty($tag->cattitlelink)) $result = '<h3 class="cattitle"><a href="index.php?option=com_content&view=category&id='.$this->currentcatid.'">'.$article->cattitle.'</a></h3>'.$result;
			else $result = '<h3 class="cattitle">'.$article->cattitle.'</h3>'.$result;
		}
	...
	public function acymailing_generateautonews(&$email){
		...
		foreach($tags as $oneTag => $parameter){
			...
			if(!empty($parameter->maxcreated)){
				...
				$where[] = 	'`created` < '.$this->db->Quote(date('Y-m-d H:i:s',$date));
			}
