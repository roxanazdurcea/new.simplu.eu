...
class EasyBlogControllerInstallCopy extends EasyBlogSetupController
{
	...
	public function execute()
	{
		...
		if ($type == 'languages') {
			...
			JFile::copy($adminFile, JPATH_ADMINISTRATOR . '/language/en-GB/en-GB.com-easyblog.ini');

			// Copy the admin system language file
			$adminFileSys = $path . '/admin/en-GB.com_easyblog.sys.ini';
			JFile::copy($adminFileSys, JPATH_ADMINISTRATOR . '/language/en-GB/en-GB.com-easyblog.sys.ini');

			// Copy the site language file
			$siteFile = $path . '/site/en-GB.com_easyblog.ini';
			JFile::copy($siteFile, JPATH_ROOT . '/language/en-GB/en-GB.com-easyblog.ini');
